...
* Der lokale IPC-Socket ist fuer den paketierten Root-Betrieb jetzt so gehaertet, dass nicht-privilegierte lokale Prozesse ueber eine world-writable Socket-Datei auf den Agent zugreifen koennen.
...
* Der Agent protokolliert bei `ROOK_AGENT_LOG_LEVEL=debug` jetzt eingehende IPC-Nachrichten sowie die daraus entstehenden IPC- und Backend-Ergebnisse, damit Operatoren den Nachrichtenfluss im Journal nachvollziehen koennen.
* Offensichtliche Secret-Felder wie WLAN-Passwoerter werden in diesen Debug-Logs vor dem Schreiben gezielt maskiert, damit Fehlersuche moeglich bleibt ohne Klartext-Passwoerter ins Journal zu legen.
* Die Netzwerk-Aktionen `ConnectWifi` und `VpnStart` beantworten erfolgreiche CLI- und IPC-Aufrufe jetzt erst dann, wenn die jeweilige Verbindung effektiv nutzbar ist und bereits eine IPv4-Adresse traegt.
