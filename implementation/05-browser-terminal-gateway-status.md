...
* Plan 06 (`WebSocket-Autorisierung per Nachricht statt Header`) wurde umgesetzt und im Zuge des ersten echten Browser-Integrationslaufs nachgezogen.
...
* Der erste echte Browser-Integrationslauf hat den letzten Folgeplan dieser Reihe ausgeloest; Plan 07 ist umgesetzt, nachgezogen und fuer den aktuellen Scope abgeschlossen:
...
* Der Browser-WebSocket-Handshake aus Plan 02 ist jetzt browser-kompatibel nachgezogen:
  ...
  * Origin-/Proxy-Regeln werden fuer den jeweiligen Zielbetrieb im Gateway-Repo gepflegt und hier nicht mehr als offener Implementierungsblocker gefuehrt
...
* Die daraus abgeleitete Betriebssemantik ist jetzt umgesetzt:
...
* Der Gateway bleibt lokal reproduzierbar testbar und war damit entlang Browser, Backend und SSH durchgaengig nachvollziehbar:
  * Mock-Backend fuer die Grant-Seite
  * lokaler Integrationspfad gegen Test-SSH-Server und WebSocket-Client unter `tests/e2e/`
* Fruehere MVP-Kompromisse wie die Host-Key-Verifikation gehoeren nicht mehr zur offenen Planreihe dieses Dokuments; weitere betriebliche Haertungen werden im Implementierungsrepo als laufende Betriebsarbeit gefuehrt.
...
* Der Paketbuild ist fuer das aktuelle Team-Setup auf lokale Vorvalidierung ohne Debian-Toolchain ausgelegt:
  ...
  * zusaetzliche Installations- und Laufzeittests auf Debian gehoeren nicht mehr zur offenen Grundplanreihe dieses Dokuments
...
1. Neue Gateway-Aenderungen nur noch als Betriebsbefund, Hardening-Massnahme oder neue Ausbauphase dokumentieren.
2. Rueckmeldungen aus spaeteren Integrationslaeufen getrennt vom abgeschlossenen Grundstand fuehren.
3. Zusatzaenderungen wie weiteres Packaging-Hardening oder betriebliche Sicherheitsmassnahmen als eigene Folgearbeit priorisieren.

## Aktuelle Integrationsbefunde

* Der erste echte Browser-Integrationslauf hat den Fokus auf Idle-, Timeout- und Keepalive-Verhalten geschaerft; die daraus abgeleiteten Aenderungen sind fuer den aktuellen Scope umgesetzt.
* Die fruehere Idle-Abbruchbeobachtung ist im Gateway in eine getrennte Autorisierungs- und Keepalive-Semantik ueberfuehrt.
* Die zentrale Einordnung der damaligen komponentenuebergreifenden Befunde bleibt in `11-integrationsbefunde-und-folgearbeiten.md` nachvollziehbar.
* Fuer dieses Statusdokument heisst das insbesondere:
  * der erreichte Keepalive- und Session-Endgrund-Stand ist Teil des laufenden Betriebs,
  * weitere Auffaelligkeiten entlang Browser, Proxy, Gateway, SSH und Backend starten als neuer Befund,
  * neue Laufzeitgrenzen oder Sicherheitsregeln gehoeren in eine spaetere Ausbauphase.
* Der repo-lokale Umsetzungsschnitt dafuer ist in `plans/07-idle-keepalive-und-session-endgruende.md` nachvollziehbar abgeschlossen.
...
* Abgeschlossene Planstufen bleiben hier in verdichteter Rueckschau sichtbar; neue Arbeiten sollen als neue Ausbauphase klar getrennt werden.
* Weitere Aktualisierungen sollen hier vor allem den laufenden Betriebsstand, spaetere Hardening-Schritte und neue Integrationsbefunde spiegeln.
